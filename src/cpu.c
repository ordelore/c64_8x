...
void cpu_dump2(cpu_t *cpu) {
    dbg_printf(" A=%hhX X=%hhX Y=%hhX S=%hhX P=%X\n",
...
void cpu_start(cpu_t *cpu) {
    uint8_t bt = mem_peek(cpu->memory, 0xFFFC);
    dbg_printf("bt: %d %x\n", bt, bt);
...
uint8_t step_cpu(cpu_t *cpu) {
    ...
    switch (cpu->ir)
    {
        ...
        case 0x00 : cpu_brk(cpu); return 1;
        ...
        case 0x08 : cpu_push(cpu, cpu->p); break; //php
