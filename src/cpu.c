...
void cpu_dump2(cpu_t *cpu) {
    dbg_printf(" A=%hhX X=%hhX Y=%hhX S=%hhX P=%hhX\n",
...
void cpu_bfs(cpu_t *cpu, uint8_t flag) {
    ...
    if (flagget(cpu, flag)) {
        cpu->pc += (int8_t)mem_peek(cpu->memory, cpu->pc) + 1;
    } else {
        cpu->pc++;
    }
}
...
void cpu_bfc(cpu_t *cpu, uint8_t flag) {
    if (flagget(cpu, flag)) {
        ...
        cpu->pc += mem_peek(cpu->memory, cpu->pc) + 1;
    }
}
...
uint8_t step_cpu(cpu_t *cpu) {
    ...
    if (low == 0x00) {
        ...
        if (cpu->ir == 0x16) {cpu_asl(cpu, cpu_zpx(cpu));}
        ...
        if (cpu->ir == 0x29) {cpu_and_(cpu, cpu_imm(cpu));}
        ...
        if (cpu->ir == 0x35) {cpu_and_(cpu, cpu_zpx(cpu));}
        ...
        if (cpu->ir == 0x49) {cpu_eor(cpu, cpu_imm(cpu));}
        ...
        if (cpu->ir == 0x50) {cpu_bfc(cpu, V);} //bvc
        ...
        if (cpu->ir == 0x69) {cpu_adc(cpu, cpu_imm(cpu));}
        ...
        if (cpu->ir == 0x75) {cpu_adc(cpu, cpu_zpx(cpu));}
        ...
        if (cpu->ir == 0x90) {cpu_bfc(cpu, C);} //bcc
        ...
        if (cpu->ir == 0xA9) {cpu_lda(cpu, cpu_imm(cpu));}
        ...
        if (cpu->ir == 0xB0) {cpu_bfs(cpu, C);} //bcs
        ...
        if (cpu->ir == 0xC9) {cpu_cmp(cpu, cpu_imm(cpu));}
        ...
        if (cpu->ir == 0xD0) {cpu_bfc(cpu, Z);} //bne
        ...
        if (cpu->ir == 0xE0) {cpu_cpx(cpu, cpu_imm(cpu));}
    ...
    {
        if (cpu->ir == 0xF0) {cpu_bfs(cpu, Z);} //beq
